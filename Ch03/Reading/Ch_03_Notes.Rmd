...
- Subset rasters with `[` syntax from base R
  - this syntax accepts Row,Column indexing; Cell ID; Coordinates; another 
    spatial object
```{r non-spatial-raster-subset}
elev[1, 1] # row 1, column 1
elev[1] # Cell ID 1
```
- Subset multi-layered objects with bracket notation returns cell values
    for each layer.
...
- modifying values of multilayered rasters requires a matrix with columns for
  layers and rows as cells to be replaced.
